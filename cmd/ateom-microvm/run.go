...
func (s *AteomService) RunWorkload(ctx context.Context, req *ateompb.RunWorkloadRequest) (resp *ateompb.RunWorkloadResponse, retErr error) {
	...
	s.actorLogger.EmitLifecycleLog("Actor starting", atespace, id)
	...
	for _, c := range ctrs {
		s.startActorLogForwarding(ac, atespace, id, overlayWorkloadID(c.name), c.name)
	}

	s.actorLogger.EmitLifecycleLog("Actor started", atespace, id)
...
// the connection.
func (s *AteomService) startActorLogForwarding(ac *kata.AgentClient, atespace, actorID, streamID, containerName string) {
	go s.actorLogger.WrapContainerLogs(kata.NewStdioReader(context.Background(), ac, streamID, streamID, false), atespace, actorID, containerName)
	go s.actorLogger.WrapContainerLogs(kata.NewStdioReader(context.Background(), ac, streamID, streamID, true), atespace, actorID, containerName)
}
