...
func checkRequestNameMatchesMethod(api *model.API) []Finding {
	...
	for _, svc := range api.Services {
		for _, method := range svc.Methods {
			...
			if !ok {
				findings = append(findings, Finding{Rule: requestNameMatchesMethodName, Subject: subject, Message: "request type " + method.InputName + " not found"})
				continue
			}
			if req.Name != want {
				findings = append(findings, Finding{
					Rule: requestNameMatchesMethodName, Subject: subject,
