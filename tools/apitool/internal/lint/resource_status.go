...
const resourceStatusFieldShapeName = "resource-status-field-shape"

// ResourceStatusFieldShape requires a resource's "status" field, if
// present, to be typed "{ResourceName}Status". A resource with no status
// field is fine - this rule only fires once one exists and is shaped
// wrong.
var ResourceStatusFieldShape = Rule{
	Name:        resourceStatusFieldShapeName,
	Description: `A resource's "status" field, if present, is typed "{ResourceName}Status".`,
	Check:       checkResourceStatusFieldShape,
}

func checkResourceStatusFieldShape(api *model.API) []Finding {
	resources, err := model.GroupByResource(api)
	if err != nil {
		return []Finding{{Rule: resourceStatusFieldShapeName, Subject: "(GroupByResource)", Message: err.Error()}}
	}
	...
	for _, rg := range resources {
		...
		if field.TypeKind != "message" || field.TypeFullName != want {
			findings = append(findings, Finding{
				Rule: resourceStatusFieldShapeName, Subject: rg.Message.FullName,
