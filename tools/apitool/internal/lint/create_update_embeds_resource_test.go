...
func TestCreateUpdateRequestEmbedsResource(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			findings := lint.CreateUpdateRequestEmbedsResource.Check(createUpdateAPI(tt.reqFields))
...
func TestCreateUpdateRequestEmbedsResource_IgnoresGetDelete(t *testing.T) {
	...
	if findings := lint.CreateUpdateRequestEmbedsResource.Check(api); len(findings) != 0 {
