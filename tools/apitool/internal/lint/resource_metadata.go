...
const resourceMetadataName = "resource-metadata"
...
var ResourceMetadata = Rule{
	Name:        resourceMetadataName,
	Description: `Every resource message has a "ResourceMetadata metadata = 1" field.`,
	Check:       checkResourceMetadata,
}

func checkResourceMetadata(api *model.API) []Finding {
	resources, err := model.GroupByResource(api)
	if err != nil {
		return []Finding{{Rule: resourceMetadataName, Subject: "(GroupByResource)", Message: err.Error()}}
	}
	...
	for _, rg := range resources {
		...
		case field == nil:
			findings = append(findings, Finding{
				Rule: resourceMetadataName, Subject: rg.Message.FullName,
