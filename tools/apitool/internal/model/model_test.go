...
func TestFilterToService(t *testing.T) {
	api := model.FilterToService(buildRealAPI(t), "Control")
	...
	for _, fullName := range []string{
		...
		// AssetFile. Proves FilterToService follows MapValueFullName, not
		// just TypeFullName.
...
func TestFilterToService_UnknownService(t *testing.T) {
	api := model.FilterToService(buildRealAPI(t), "NoSuchService")
	if len(api.Services) != 0 || len(api.Messages) != 0 || len(api.Enums) != 0 {
		t.Errorf("FilterToService(unknown) = %+v, want a completely empty API", api)
	}
}
...
func TestGroupByResource(t *testing.T) {
	...
	groups, err := model.GroupByResource(api)
	if err != nil {
		t.Fatalf("GroupByResource() error = %v", err)
	}

	var actorGroup *model.ResourceGroup
	for i := range groups {
	...
	if actorGroup == nil {
		t.Fatalf("no ResourceGroup for Actor; groups = %+v", groups)
	}
...
func TestGroupByResource_InvalidMethodName(t *testing.T) {
	tests := []struct {
		name       string
		methodName string
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if _, err := model.GroupByResource(api); err == nil {
				t.Errorf("GroupByResource() error = nil for method %q, want an error", tt.methodName)
			}
		})
	}
}
