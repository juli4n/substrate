...
The duration keys are the [`ate.actor.restore.duration`](#the-metric-registry) instrument's name with an `ate.snapshot.phase` value appended, and they hold **seconds**, matching that instrument's declared unit. The same phase rules apply as on the histogram: a phase that never ran is absent rather than zero, and phases overlap and do not sum to the total. `ate.failure.reason` is present on this record only when the restore failed — unlike the histogram, which carries no failure label at all (see [Which side failed](#which-side-failed-atefailuredomain)). There is no `ate.snapshot.phase` key on the record — on a datapoint it names the one step timed, and this record carries them all.
...
`ate.failure.reason` names the cause; `ate.failure.domain` names the side of the platform boundary it came from, as `infrastructure` or `unknown`. It rides on every signal that carries a reason, and the two are always emitted together — producers call `ateattr.FailureAttributes` or `ateattr.FailureLogAttrs` rather than setting either key directly.

The domain is a strict function of the reason, so it costs no series and no consumer needs it to disambiguate a value. `ate.failure.reason` only ever takes a specific value for a failure the control plane detects itself (a corrupted assignment, a worker gone, a worker reassigned) — every atelet RPC failure crashes the actor unconditionally and reports `UNKNOWN`, since the cause does not cross the atelet-to-ateapi process boundary. `UNKNOWN` therefore reports `unknown` and not `infrastructure` — a gap in the taxonomy stays visible instead of inflating one side.
...
Neither histogram carries a failure label: atelet's own failures have no cause of their own to report (every one would read the same constant `UNKNOWN`/`unknown`, per [Which side failed](#which-side-failed-atefailuredomain)), so attaching `ate.failure.reason`/`ate.failure.domain` here would add no information, only a misleading appearance of one. The [`Restore timing breakdown`](#actor-attributed-component-logs) log record carries `ate.failure.reason` on a failed restore instead, where cardinality is free and the constant value at least marks *that* the phase failed.
